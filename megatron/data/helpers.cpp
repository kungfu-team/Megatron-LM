...
#define LOG(fmt, ...) PRN("%s: %s" fmt, __FILE_NAME__, __func__, __VA_ARGS__)
                            ...
                            const bool verbose)
{
    ...
    if (verbose) {
        std::cout << "> building indices for blendable datasets ..."
                  << std::endl;
    }
    ...
    for (int64_t sample_idx = 0; sample_idx < size; ++sample_idx) {
        ...
        for (int64_t dataset_idx = 1; dataset_idx < num_datasets;
             ++dataset_idx) {
    ...
    if (verbose) {
        ...
        for (int64_t dataset_idx = 0; dataset_idx < num_datasets;
             ++dataset_idx) {
    ...
    */
    ...
    for (int32_t iteration = 0; iteration < 2; ++iteration) {
        ...
        for (int32_t epoch = 0; epoch < num_epochs; ++epoch) {
            if (epoch % 10000 == 0) {
                LOG(" epoch: %d/%d", (int)epoch, (int)num_epochs);
            }
            LOG(" %d/%d", (int)map_index, (int)max_num_samples);
            ...
            for (int32_t doc = 0; doc < (docs.shape(0) - 1); ++doc) {
                ...
                if ((num_remain_sent >= min_num_sent) &&
                    ...
                    for (auto sent_index = sent_index_first;
                        ...
                        // and if we have at least the minimum number of
                        // sentences. or if we have reached end of the document.
                        if (((seq_len >= target_seq_len) &&
                            ...
                            if (second) {
                                ...
                                // Each sample has 4 items: the starting
                                // sentence index, ending sentence index, the
                                // index of the document from which the block
                                // comes (used for fetching titles) and the
                                // unique id of the block (used for creating
                                // block indexes)
                                ...
                                maps[map_index_0 + 1] =
                                    static_cast<DocIdx>(sent_index + 1);
                                maps[map_index_0 + 2] =
                                    static_cast<DocIdx>(doc);
                                maps[map_index_0 + 3] =
                                    static_cast<DocIdx>(block_id);
                            }
